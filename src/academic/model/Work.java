...
public class Work {
    ...
    private static int idxCrsOpen,idxEnrl;
        ...
        for(int i = 0; i < countDosen; i++){
            for(Lecturer l : Lctr){
                if(l.getInitial().equals(list[i])){
                    ...
                    LecList = LecList.concat(list[i]);
                    LecList = LecList.concat(" ");
                    LecList = LecList.concat("(");    
                    LecList = LecList.concat(temp);
                    LecList = LecList.concat(")");
                    continue;
                }
            }
        ...
        for(Enrollment E : Enrl){
            if(E.getCrs().equals(pecahan[1]) && E.getStd().equals(pecahan[2]) && E.getYear().equals(pecahan[3]) && E.getSem().equals(pecahan[4])){
                    ...
                    if(E.getStatus().equals("Remed")){
                        ...
                        simpan = simpan.concat(E.getGrade());
                        simpan = simpan.concat("(");
                        simpan = simpan.concat(temp);
                        simpan = simpan.concat(")");
                        E.setStatus("Remed");
                        E.setTemp(simpan);
                    }
